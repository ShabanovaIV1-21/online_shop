    ...
    <div class="row mb-5">
        <?php if ($subcategories !== null): ?>
            <?php foreach ($subcategories as $val) : ?>
                <h3><a class="list" href="/online_shop/categories/<?=$val->getId()?>"><?=$val->getTitle()?></a> <?=$val->getDescription()?></h3>
            <?php endforeach; ?>
        <? endif; ?>
            ...
            <?php foreach ($products as $val) : ?>
                    <div class="col-12 col-sm-6 col-lg-3 mt-4 mb-2"><!--  сколько кололонок занимает элемент внутри и при какой ширине экрана-->
                        <div class="card" >
                        <a href="/online_shop/product/<?=$val->getId()?>">
                            <img src="/online_shop/img/products/<?=$val->img?>" class="card-img-top py-2" alt="Фото продукта">
                        </a>
                            <div class="card-body">
                                <div class="card__body-up">
                                    <a style="text-decoration:none; color:black" href="/online_shop/product/<?=$val->getId()?>">
